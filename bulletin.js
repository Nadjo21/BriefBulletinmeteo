...
document.getElementById("titre1").addEventListener("click", function () {
    ...
    //on fait une boucle pour supprimer le formulaire selection des dates
    ...
    fetch(baseApiUrl + '/last?measure-type=' + measureType).then(function (response) {
        response.json().then(function (result) {
            ...
            //on initialise l'objet Date (pour en modifier le format par la suite)

            const date = new Date(result.measureDate);

            // on creer les paragraphes contenant les valeurs de l'objet RESULT
...
document.getElementById("titre2").addEventListener("click", function () {
    ...
    fetch(baseApiUrl + '/top?measure-type=' + measureType).then(function (response) {
        response.json().then(function (result) {
            ...
            // Création des paragraphes contentant les valeurs de l'objet RESULT
...
document.getElementById("titre3").addEventListener("click", function () {
    ...
    //on fait une boucle pour supprimer le dernier formulaire  selection des dates( et ne pas en ajouter de nouvelles)

    const sectionForm = document.getElementById("Selection-date");
    const elementNumberDate = sectionForm.childElementCount;
    for (let i = 0; i < elementNumberDate; i++) {
        sectionForm.removeChild(sectionForm.firstElementChild);
        console.log(i);
    }

    // Création du formulaire pour renseigner les plages de dates
    ...
    // On créé un évenement lorsqu'on clique que le bouton Rafraîchir les données
